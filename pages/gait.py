...
# # st.title("Video Gait Analysis Uploader")

# # # Use st.session_state to control the file uploader's state
# # uploaded_file = st.file_uploader(
# #     "Upload a video file for analysis",
# #     type=["mp4", "mov", "avi"], # Add more types as needed
# #     key="video_uploader", # Give it a unique key
# #     on_change=lambda: st.session_state.update({"uploaded_file": st.session_state.video_uploader})
# # )

# # # Process the file if it's new or still in session state
# # if st.session_state.uploaded_file is not None:
# #     st.success(f"File uploaded: {st.session_state.uploaded_file.name}")
# #     st.video(st.session_state.uploaded_file) # Display the video

# #     # Placeholder for your analysis logic
# #     st.write("Perform your gait analysis here...")
# #     # Example: Save the file temporarily for processing (if needed)
# #     # with open(os.path.join("temp_videos", st.session_state.uploaded_file.name), "wb") as f:
